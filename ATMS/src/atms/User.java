...
/**
 * The `User` class represents a user in the ATM system. Each user has a unique account number, a Personal
 * Identification Number (PIN), and a list of associated accounts. Users can have multiple accounts, such as
 * chequing and savings accounts. The class provides methods to manage user accounts, select a current account,
 * 
 * withdraw money, and track whether the user has performed a withdrawal.
 * - Store user attributes such as account number, PIN, and associated accounts.
 * - Add and retrieve accounts from the user's list of accounts.
 * - Select a current account based on its ID.
 * - Withdraw money from the user's currently selected account, enforcing minimum withdrawal amount.
 * - Track whether the user has performed a withdrawal.
 * - Provide a static method to get the list of all users in the system.
 * **/
